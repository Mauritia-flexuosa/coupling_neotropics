...
source("/home/mcure/Documents/Dexter/anomalia/classificação.R")
```
...
### We selected samples at the north of latitude 20ºS.
...
```{r echo=FALSE}
data.frame(Variables = colnames(dados_kmeans[,-c(1,8,9,12,13)]))
...
#### We also performed a Principal Component Analysis with the variables. The first two components explain 78.77% of variance in data.
...
### Distribution of leaf flush regimes are mixed within groups and did not present a consistent separation among these communities with the variables used in the k-means algorithm. The k-means clusterization with all variables (except those showing the annual cycle) did not separate distinct leaf flushes in its second group. For example, the number of evergreens attributed to the second group are close to the number of deciduous and semideciduous communities.
...
month <- rep(colnames(mon.df[,-dim(mon.df)[2]]), dim(mon.df[,-dim(mon.df)[2]])[2])

dataa <- reshape2::melt(mon.df[,-13]  %>% 
...
dataa <- dataa %>%
  add_column(Leaf_flush = rep(completoRN1$Leaf_flush, 12)) %>% 
...
### The performance of the k-means algorithm using the variables that resume the annual cycle of EVI2 and its oscillation, mean, entropy and so on, was not so good according with the mean silhouette Score.

```{r echo=FALSE, warning=FALSE, message=FALSE}
silhouette <- silhouette(km.res$cluster, dist(scale(dados_kmeans[,-c(1,8,9,12,13)])))
mean_silhouette <- mean(silhouette[, "sil_width"])

mean_silhouette %>%
  data.frame %>%
  rename(`Mean Silhouette Score` = ".") 
```
...
### We performed a random forest to test if the selected variables are good predictors of the leaf flush regime classification based on species composition.
...
```{r, echo=FALSE}
data.frame(Variables = colnames(dados_kmeans1[,-c(1,8,9,12,13)]))
```
...
### Mean EVI2 in *September*, *October* and *August* are the most important variables to classify leaf flush regimes in accordance with the floristic-based classification.
...
### C-means clusterization allowed us to separate floristic-deciduous and evergreen communities, while semideciduous ones became well represented in both groups. However, despite being a minority, there are deciduous communities that behave like evergreens and vice versa.
...
(data_fuz %>% 
  ...
  add_column(lon = completoRN1$lon) %>%
  ...
  theme(axis.title.y = element_text(size = 12),
        axis.title.x = element_text(size = 12),
        axis.text.y = element_text(size = 10)))/(reshape2::melt(mon.df[,-dim(mon.df)[2]]  %>% 
                         add_column(plot_id = completoRN1$plot_id),
                       id.vars = "plot_id",
                       variable.name = "month",
                       value.name = "evi2") %>%
...
### The first group shows the highest amplitude in the mean EVI2 in accordance with deciduous vegetation behavior for all leaf flush regimes previously asigned for each community. The first group shows that even evergreen dominated communities may behave as deciduous one, but presenting mean EVI2 values higher than those of both deciduous and semideciduous communities. The first group also presented a lower mean EVI2 than the second group. In the second group, all types of communities have a lower oscilation than those from the first group. Evergreen communities, in the second group, present an increase in the mean EVI2 from May to October, while deciduous and semideciduous communities increase greenness in October and September respectively.

```{r echo=FALSE, warning=FALSE, message=FALSE}
month <- rep(colnames(mon.df[,-dim(mon.df)[2]]), dim(mon.df[,-dim(mon.df)[2]])[2])

data <- reshape2::melt(mon.df[,-dim(mon.df)[2]]  %>% 
                         add_column(plot_id = completoRN1$plot_id),
                       id.vars = "plot_id",
                       variable.name = "month",
                       value.name = "evi2")
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
data_vid <- completoRN1 %>%
  pivot_longer(cols = c("lag.0","lag.1","lag.2","lag.3","lag.4","lag.5","lag.6"), values_to = "Lag") %>% 
  add_column(group = rep(data_fuz$group,7)) 


data_vid <- data_vid %>%
  mutate(name = substr(data_vid$name, 5, 5) %>% as.numeric)

data_vid %>% 
  filter(name <=4) %>% 
  ggplot(aes(x = factor(name), y = Lag, fill = factor(Leaf_flush), alpha = 0.6))+
...
data_fuz %>% 
  add_column(map = completoRN1$map) %>% 
#  filter(coupling >= 0) %>% 
...
(data_fuz %>% 
  ...
  theme(axis.title.y = element_text(size = 12),
        ...
        axis.text.y = element_text(size = 10)))/(data_fuz %>% 
  add_column(lat = completoRN1$lat) %>% 
  add_column(lon = completoRN1$lon) %>%
  filter(coupling < 0) %>%
  ggplot(aes(x = lon, y = lat, color = factor(Leaf_flush), alpha = 0.5))+
